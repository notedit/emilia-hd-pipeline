...
- **`phase1-filtered` 分支**（短路发布，不等 Phase-2）：WebDataset shard
  （`{clip_id}.mp3 + {clip_id}.json`，trim 已应用）+ `metadata/phase1_metrics.parquet`
  （每 clip 一行全量 S0-S3 指标 + `tier` + `sample_rate`，`clip_id` 主键）；
- **Phase-2 标签增量**：`upload_s4_labels()` 只上传 `metadata/s4_labels.parquet`
  （同 `clip_id` 键，随打标进度反复执行），**音频 tar 永不重写**；
- **`main` 分支**：S5 完成后的完整分层版。

#### 发布分层（`hf.publish_scope`，2026-07 方案 C）

物理发布全部 S1 幸存者，按 tier 分目录 `data/{tier}/`，下游按需下载、
更严/更松的门槛都是 parquet 查询，永远不需要重打包重传：

| Tier | 筛选规则 | 用途 |
|------|----------|------|
| `data/prime/` | S0 ∧ S1 ∧ S3 通过（single / intruded_trimmed / degraded_pass）∧ `prosody_dsp_score` 位于 S3 通过集合的**全局 top 40%** | 精品表现力核心集，只要最好的就只下这层 |
| `data/extended/` | S0 ∧ S1 ∧ S3 通过，韵律分在 40% 线以下 | 干净但韵律平淡；要量时与 prime 合用 |
| `data/s3rejected/` | S0 ∧ S1 通过、S3 声纹纯度拒绝（可能混入他人语音）；**原样音频，不剪裁** | 风险自担；S3 有过杀（听感验证长句/高表现力被误伤），阈值调优后可从这层召回 |

要点：`prosody_dsp_score` 的 z-score 母体是全部 S1 幸存者，分数与发布范围无关；
`publish_scope: prime|s3|s1` 控制物理发布集（`prime` 即旧行为）；音频保留
**原始采样率**（Emilia-ZH 混采 24/32/44.1 kHz，见 `sample_rate` 列），管线从不重采样
（16 kHz 只是过程中喂模型的内存副本）；全库统一 **mp3**（原样 clip 是源字节，
`intruded_trimmed` 修剪后以高码率 VBR mp3 重编码——shard 内编解码同质，
`datasets.load_dataset("webdataset")` 才能直接加载）；打包为流式（逐源 tar，
`hf.pack_workers` 并行 fork），内存峰值与规模无关。
